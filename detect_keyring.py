...
is_mac_keychain = ('OSX' in kr_name or 'Keychain' in kr_name or 'macOS' in kr_name or 'keychain' in kr_module.lower())
...
try:
    ...
    print(' - For CI or headless usage consider using your platform secret store or CI secret variables.')
except Exception:
...
if security_path:
    print("Example: store a generic password:\n  security add-generic-password -a my-account -s my-service -w 'my-secret' -U\nRetrieve it:\n  security find-generic-password -s my-service -w")

print('\nIf an application cannot detect a keyring:')
print(' - For Python apps: install `keyring` and optionally `keyrings.alt`, or set `PYTHON_KEYRING_BACKEND` to an encrypted file backend.')
print(' - For Node/Electron: ensure `keytar` is installed and the process has access to Keychain; otherwise use environment secrets or an encrypted file vault.')
